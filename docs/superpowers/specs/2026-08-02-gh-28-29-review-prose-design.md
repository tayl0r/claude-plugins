...
- `plugins/dev-flow/.claude-plugin/plugin.json`: **2.7.0 → 2.8.0**
- `plugins/dev-flow-worktree/.claude-plugin/plugin.json`: **1.9.0 → 1.10.0**

**Amended after the base moved.** This change was designed and executed against base `c8b2182`, where the versions were `2.6.0`/`1.8.0`, and it originally targeted `2.7.0`/`1.9.0`. While it was in flight, PRs #35 and #36 merged and took exactly those numbers. Merging `origin/main` into this branch **silently auto-resolved** the version lines — both sides had changed `2.6.0` → `2.7.0`, so git saw no conflict — which would have shipped this change's `SKILL.md` text at a version already published *without* it, and the version-keyed install cache would never have picked it up. That is precisely the failure the bump rule exists to prevent, and it is invisible to `check-sync.py`, to `claude plugin validate`, and to every one of the success criteria below, none of which compares against the *published* version. Targets re-derived from the new base, same minor-bump reasoning. **Success criterion 7 is updated to `2.8.0`/`1.10.0` accordingly**, and criterion 8's `BASE` — always computed via `merge-base`, never hardcoded — now resolves to the merge base with the advanced `main`.
...
7. `plugins/dev-flow/.claude-plugin/plugin.json` has `"version": "2.8.0"`; `plugins/dev-flow-worktree/.claude-plugin/plugin.json` has `"version": "1.10.0"`. **Additionally — the check the original criteria lacked — each must be strictly greater than the version at `origin/main`**, so a concurrently-merged bump cannot silently consume this change's numbers: `git show origin/main:<path>` must read `2.7.0` and `1.9.0` respectively, or lower.
