...
```
- **Make each `## Task N` section self-sufficient — instruct `writing-plans` so.** `scripts/task-brief` hands each SDD implementer only the text between `## Task N` and the next task heading, with no plan-file path — so any reference a task leans on from **outside that span** (a shared verification block, a bare `§`/`§V` ID, another task's output, an implicit "the table above") is unresolvable from the brief. The trigger is **structural** — the target is not defined within the section — not how explicit the reference reads. Require every such reference to be **inlined into the task section, or named there by the plan's absolute path** with this clause: *read the referenced block verbatim from the plan file at `<abs-path>`; never reconstruct or substitute it; if you cannot read the plan file, stop and report.* A pointer in `## Global Constraints`, a `## Verification scripts` preamble, or any non-`## Task N` section is stripped by `task-brief` and never reaches the implementer. This doubles as a `writing-plans` self-review criterion: **halt and report** any out-of-section reference that cannot be given an in-section pointer rather than ship the plan.
```
