...
- **"which a worktree-isolated runner refuses without running"** is stated as the *secondary* reason, after the by-construction one. Whether a given agent is worktree-isolated is a harness fact that varies; that `argv` cannot word-split does not.
...
- **A6. The runner-refusal predicate is a harness fact, measured today and not pinned by any check here.** The table under *#40's comment supersedes the naive fix* records the measurement and the date. Block 1 is worded so that the by-construction argument carries the rule on its own: if the harness later accepts the shell form, the `argv` rule is still correct and the secondary clause is merely no longer motivating.
