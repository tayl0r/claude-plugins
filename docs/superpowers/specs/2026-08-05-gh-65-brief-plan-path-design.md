...
gh-45 (PR #64, shipped) added two **prose** defenses at plan-authoring time: a Stage 2 rule telling the plan author to make each `## Task N` section self-sufficient, and a one-line sharpening of the `adversarial-review` plan-mode reviewer. Both are non-mechanical — they depend on the plan author remembering to carry the resolver into every referencing section.

This design records the **cause-fix** at the one seam this repo actually controls: **dev-flow's orchestrator is SDD's controller**, and in Stage 3 it composes each implementer brief itself. So a new Stage 3 override makes the orchestrator include in *every* implementer brief the plan's absolute path plus a resolve-out-of-section-references-verbatim clause. The resolver rides on every brief, so correctness stops depending on the plan author anticipating each reference — it is **correct-by-default**.
...
**Rejected — edit `task-brief` upstream.** It is the platonic shared seam — every SDD user would inherit it, and it is the ideal home — but it is an external superpowers artifact this repo cannot edit. Recorded as the ideal seam; upstream escalation is a tracked follow-up (this design files nothing upstream).
...
- **Editing any external superpowers artifact** — `subagent-driven-development`, `scripts/task-brief`, `implementer-prompt.md`. Not in this repo; cannot be changed here. Upstream `task-brief` escalation is a tracked follow-up, filed by no part of this change.
