...
**Pending probe (required before implementation).** One load-bearing capability is asserted but not yet field-verified: that `EnterWorktree(path)` is accepted from a **depth-2** cwd-pinned agent (main → stage subagent → fixer). `EnterWorktree`'s documented contract supports path-entry from pinned agents for same-repo `.claude/worktrees/` targets, but doc-vs-runtime drift is the exact failure class behind this whole design, so a probe must confirm it: from a depth-2 sub-subagent, `EnterWorktree` a scratch worktree under `.claude/worktrees/`, then write and commit there. **If it fails, the fallback is mechanical, not architectural** — fixers address the threaded path with `git -C <path>` and absolute file paths (see Worktree seam). The probe decides the mechanism the plan writes down; it does not gate the architecture.
...
3. **Read-only** reviewers (seeds, resolvers) receive absolute artifact/diff paths derived from the resolved root and need no entry. **Write-side** fixers enter the root explicitly: `EnterWorktree(path)` when it is under `.claude/worktrees/` (the dev-flow case — the only location that tool accepts from a pinned agent), else `git -C <path>` with absolute file paths (the standalone case — `EnterWorktree` *rejects* non-`.claude/worktrees/` targets, so threading + explicit addressing is the only rule that works in both modes; entry is an optimization where available).
...
- **`adversarial-review/SKILL.md`** — add to the **Contract**: (i) reviewer passes run as separate subagents on their models, halt if unspawnable, never inline (Fix 2, first home); (ii) reviewer model self-report + verification, and the provenance field in report-back (Fix 3); (iii) the working-dir rule — resolve once (arg, else invoking checkout root, absolute), thread to every spawned agent, write-side enters via `EnterWorktree` under `.claude/worktrees/` else `git -C`, argument is an override not the mechanism (Worktree seam). Leave the two-tier protocol and model policy otherwise untouched.
