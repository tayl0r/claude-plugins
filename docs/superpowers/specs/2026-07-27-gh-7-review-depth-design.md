...
## Scope check — one change: two families, six files

Two decomposition questions have to be asked, not one. The issue holds two genuinely independent problems, and the change touches six files across four kinds of artifact. Both answers are **one change**, for different reasons, and neither reason is convenience.

**Are the two families one change?** Different stage targets (design/plan vs. diff), different seed passes (correctness vs. quality), different risk profiles (Family 1 is a low-risk enumeration; Family 2 is the one that can turn into a false-positive machine). The question is serious. The answer is yes:

- **They land in the same file pair, in the same section, and share one architectural decision.** Both edits are inside `## Seed passes` of the `adversarial-review/SKILL.md` mirror pair. Both are resolved by the same non-obvious ruling — *seed-side, not rubric-side* (see both families' decisions below). Splitting means establishing that ruling twice, and the second design would be mostly a restatement of the first.
- **Splitting is strictly more expensive with no isolation benefit.** Two version bumps of the same two plugins, two PRs mutating the same `check-sync.py`-enforced mirror pair within nine lines of each other, and a guaranteed rebase. The mirror pair's line-parallel schema makes overlapping edits in one region the exact thing that is annoying to land twice.
...
**Does every file belong in the same change?** Six files is four groups, and each is in for its own reason — they are not uniformly coupled:

- **The two `adversarial-review/SKILL.md` copies** are the change. A declared mirror pair: editing one without the other fails `check-sync.py`.
- **The two `plugin.json` version bumps** are not a severable change at all. The install cache is version-keyed, so an edit shipped at an unchanged version is never picked up (§3 & 4). A bump without the edit means nothing; the edit without a bump never reaches a user. There is no second PR that could hold them.
- **`CONTEXT.md` must land in the same *commit*, not merely the same PR** — the tightest coupling of the four, because both orderings are actively harmful. `docs/agents/domain.md` has every skill read the glossary before exploring, use its terms, and treat a missing term as a signal it may be inventing language to reconsider. Early, an agent hunts for a fifth angle that does not exist; late, an agent reads "four angles" against a file that ships five and is instructed to doubt the fifth. `main` has already adjudicated this: #17 removed exactly these entries for landing early, on the stated grounds that they return with the behaviour. Splitting re-opens a merged decision with no new evidence. Full argument in §6.
- **`CLAUDE.md` is the one genuinely severable item, and it stays.** It ships nowhere, changes no behaviour, and needs no version bump; it could be a one-line PR of its own. It stays because it is the *shared-boundary* fix for a gap this change is the evidence of: the merged rule binds only hand-mirrored edits, this change edits no hand-mirrored file, and its verification has to apply the technique anyway to the machine-checked pair (see Verification's closing note). Split, this PR visibly ignores a merged `CLAUDE.md` rule while PR-2's whole justification points back here — so PR-2's reviewer loads this context regardless and the isolation buys nothing. Deferring a shared-boundary fix to a follow-up nobody has scheduled is the latent regression the rubric names. Full argument in §5.

This is not a case of independent subsystems being forced together, so no halt is warranted. Family 2's risk is addressed by designing its trigger properly (below), not by shipping it separately; the file count is one behaviour change carrying its own version keys, its own vocabulary, and the verification lesson it taught.
...
`plugins/dev-flow/skills/adversarial-review/SKILL.md` is 1,900 words today and ships into every review invocation of both plugins. The two additions total 314 words, plus a net +11 on the angles-block header and ~11 in the two table cells — about **+17%** of the file, ~555 tokens.
...
So instead of appending a second, differently-scoped rule for the design-conformance check, this change **replaces** the first with one rule that states the root cause and names both techniques as instances of it — one that always applies, one gated on the change having a design doc — scoped to any mirrored pair. That edits already-merged adjacent text, which the rubric permits when it reaches the better design — and here it corrects a scoping error rather than merely tidying. It also stops the accretion at two: the next verification technique is another instance of a stated cause, not a third bullet.
...
`CLAUDE.md` stays at **29 lines** — this is a one-line replacement, not an append. `check-sync.py` never reads this file (it names it only in error strings), and no plugin ships it, so the rule costs no invocation context. A generic runner is deliberately not built: the block-to-file mapping and the shape assertion differ per change (`gh-10`: eight one-line blocks; this change: `[1, 1, 1, 2, 2, 1, 12]`), so factoring it out would need a machine-readable annotation schema on every design doc, which two instances do not justify.
...
3. **Residue grep — the two in-place edits left no old text behind:**
   ...
   Expect **no output** (exit 1). Each of the five in-place replacements removes exactly one of these substrings — two table cells, the angles-block header, the `Pass` entry in `CONTEXT.md`, and the sentence `gh-10` appended to `CLAUDE.md`'s mirroring bullet.

   That last anchor is load-bearing and not symmetric with the others. Step 6 asserts each block is *present* in its target; it never asserts the text a replacement removes is *absent*. So a `CLAUDE.md` edit that appended §5's rule as a new line instead of replacing line 9 would leave both the old and new sentences in place and still pass step 6, `check-sync.py`, and step 4. Only this grep sees it. `check-sync.py` cannot cover this: it compares the two copies *to each other*, so an edit missed on **both** sides leaves them identical and passes clean. Only a residue grep sees that. `-F` is deliberate — the patterns contain `(`, `)`, and `|`, and a fixed-string match avoids regex interpretation. The pathspec excludes the immutable historical specs and plans; `git grep` reads tracked files only, so the git-ignored `.superpowers/` scratch is excluded automatically.
...
```
   ...
   Expect exactly `design-conformance: OK` and `exit=0`. A `MISMATCH` line names the block and the file whose text or position differs — re-paste that block from §Exact change list and re-run from step 1. The shape assertion (`[1, 1, 1, 2, 2, 1, 12]`) fires if this document's plain-fenced blocks are ever added to, removed, or reflowed: that is deliberate, because the blocks are indexed positionally. Every fenced block in this Verification section carries the `sh` info string and is therefore skipped by the `mode == ""` filter, so adding or editing a verification step never disturbs that index — keep it that way.
