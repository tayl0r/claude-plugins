...
### Edit 3 — discharge the section, in a dedicated **merge-gate step 4**

**Anchor (both twins):** merge-gate **step 3**, the bullet beginning `3. **Consult `stops`**` (dev-flow `:253`, worktree `:247`), ending `… step 1 already halted it.)`. Leave step 3 as its plain `Consult `stops`` and **insert a dedicated new step, `4. **Discharge the merge-gate checks.**`, between it and the strip**; then **renumber the strip `4 → 5` and the merge `5 → 6`** and bump every merge-gate cross-reference that names them — the intro's re-entrancy note (`step 4 → step 5`), the discharge's own `step 4's strip → step 5's strip`, and the gate's `five steps → six steps`. **Do not touch the two worktree-entry `step 4` references:** the twin intro's and the discharge's trailing clause both say *worktree entry, step 4*, which names the *worktree-entry* procedure's step 4, not the merge gate's — a deliberate landmine that stays. A dedicated step is the right altitude — the merge gate is the densest section in the spec, and one numbered step with one responsibility reads far better than cramming the discharge onto step 3's already-long line. *Rejected alternative:* extend step 3 in place and rename its header to a compound (`Consult `stops`, then discharge …`) to dodge the renumber — rejected because the renumber is a handful of mechanical, greppable `step N` edits (applied here with a per-reference assertion each), a one-time cost buying a permanently more readable gate. The discharge belongs at step 4 for the same reason it sat in step 3 before: it runs after the `pre-merge` pause (step 3) and **before any strip** (step 5), and its stripped-state handling mirrors step 3's — "in the stripped state there is no doc at tip and this discharge is not attempted."

Step 4 states: **after** step 3's stop consultation (so it runs on the resuming, non-paused pass — or immediately, when there is no `pre-merge` stop) and **before** proceeding to step 5's strip, the orchestrator discharges the plan's `## Merge-gate checks` section against a **freshly-fetched `origin/main`**. The shipped prose carries the pinned literal `freshly-fetched` (success criterion 6) and, for the stripped-state clause, the plain literal `this discharge is not attempted` (criterion 10) — both written verbatim, no interior emphasis, so the fixed-string greps match:
...
- In the **stripped state** the plan doc is gone at tip and this discharge is not attempted — it ran on the pre-strip pass, exactly as step 3's `stops` read is not attempted there. (Under `docs: commit`, the default and this repo's setting, there is no strip: step 4's discharge is immediately followed by step 5's strip no-op and step 6's merge, so the check runs against `origin/main` seconds before the merge.)
- One clause naming what this buys: the marker and CI wait certify facts anchored to the branch SHA; this is the only re-verification of facts anchored to the *moving* `origin/main`, and it is what makes the gate's re-entrancy invariant true of the design's criteria and not only of the gate's own six steps. In the **worktree** twin, this discharge — like every git command in that gate — runs from inside the pipeline worktree (the twin's intro already says so).
