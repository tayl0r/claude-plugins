...
- **Plan-review preservation:** the adversarial-review rewrite contract (line 75) currently preserves only "the doc's front-matter block and its `## Original problem` section unchanged" — extend it to also preserve `risk:` lines in `## Task N` sections, so a plan review that touches a marked task's section cannot silently drop the marker. (The plan review's correctness seed is the backstop: it flags a `risk: high` marker that the rewrite dropped.)
- **Plan-review marker backstop:** the plan review's correctness seed checks that every `## Task N` section whose work touches concurrency, auth, state machines, or other subtle invariants carries a `risk: high` marker — so a produce-subagent that forgets to mark a risky task is caught at the plan review, not at Execute.
