...
with:
`**Design / plan docs:** rewrite the doc incorporating the resolutions, **preserving the doc's front-matter block, its \`## Original problem\` section, and every \`risk: high\` line in \`## Task N\` sections unchanged** (caller state, not review content), and commit the rewritten doc on the working directory's branch.`
