...
The orchestrator spawns produce-subagents, executors, and bookkeeping agents on the main session model. Reviewer-model selection — the orchestrator spawns the review's seed/resolver leaves directly, on a capable model different from the artifact's author — is owned by `dev-flow:adversarial-review` and stated once, in its Model section; that rule travels with the review skill wherever it is invoked.
...
1. Spawn one `sonnet` leaf and one `fable` leaf (or `opus` in a Fable-family session, per `dev-flow:adversarial-review`'s Model section), each returning the model its system prompt names.
2. If either fails to spawn or reports the wrong tier (by family match — e.g. a "Fable 5" report satisfies `fable`), **halt** with a report naming the missing model — before any draft is written and later discarded on resume.
...
4. **Enter:** the orchestrator routes the resume table from the main checkout (reads via `git show dev-flow/<slug>:<file>`, branch listings, and `gh` — none need entry), then `cd`s into the worktree to *drive* a stage's in-context work — an `adversarial-review` invocation or the Execute SDD loop — so in-context git and scripts (baseline/per-task suites, `HEAD`-relative commands, the review's post-fix suite) run against the pipeline tree; it `cd`s back to main for the next routing read. Spawned leaves are pinned to the repo root and cannot inherit cwd, so each is handed the absolute worktree path explicitly: `adversarial-review` via its `working-dir` argument (write-side fixers use `git -C <path>`), SDD implementers via their `Work from:` field (they `cd` there before any commit — a bare `git commit` would land on the main checkout's branch). Harness worktree-entry (`EnterWorktree`) is not used — the probe found it rejected from repo-root-pinned subagents.
...
- The **orchestrator** invokes `dev-flow:adversarial-review` (mode: `plan`) in-context (worktree as `working-dir`); the review rewrites the plan and commits it on the branch; the orchestrator checks the provenance line. Nothing to apply afterward.
