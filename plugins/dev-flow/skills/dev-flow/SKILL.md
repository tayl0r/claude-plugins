...
- **Merge:** `gh pr merge <pr> --squash` — deliberately **without** `--delete-branch`. Branch teardown lives in exactly one place, the idempotent Cleanup below, because the resume table's Done row re-runs Cleanup on its own (e.g. resuming after a merge that already happened), and a single re-runnable deletion path is simpler to reason about than splitting teardown between the one-shot `gh pr merge` and Cleanup. (`--delete-branch` would actually work here — unlike the worktree variant there is no competing worktree, so `gh` could switch the checkout off the branch and delete it — but using it would duplicate the teardown Cleanup already owns.)
- **Cleanup (idempotent — the resume table's Done row runs this same block; every step treats "already gone" as success):**
  ...
  2. Delete the local branch — **only if it is ours** (its history carries our design doc, per Branch ownership; a legitimately merged feature always does, so this guard only ever refuses a branch the pipeline never owned): `git branch -D <username>/<slug>`; treat "branch not found" as success. It must be `-D` — after a squash merge the branch is never "fully merged" in ancestry terms, so `-d` always refuses — and it must come after step 1, because git refuses to delete the currently checked-out branch.
  3. Delete the remote branch (same branch, ownership confirmed in step 2): `git push origin --delete <username>/<slug>`; treat "remote ref does not exist" as success (GitHub's auto-delete-head-branches may have won the race).
