...
- **Isolation is the feature branch, not a worktree.** dev-flow does not use git worktrees — it checks the `<username>/<slug>` branch out in your current working directory and works there (see Artifact Contract). No stage creates a worktree. If SDD lists `superpowers:using-git-worktrees` as a required workflow skill, it is **suppressed/pre-answered**: the orchestrator invokes SDD already on the dedicated feature branch, which *is* the isolation, so there is no separate workspace to create — do not let that skill spin up a worktree. Its "tests fail -> ask" gate is covered by the red-tests corollary above.
...
Intake collision check: qualify the new slug if `<username>/<slug>` already exists (local or origin) with a committed design describing a different feature, **or** if no branch exists but `gh pr list --head <username>/<slug> --state all` is non-empty — a slug whose PR history belongs to an earlier shipped feature is retired, so the resume table's PR-state reads stay unambiguous forever. (This replaces fuzzy topic-matching with mechanical existence checks.)
...
2. **Dirty-checkout gate (before any switch).** A switch onto the feature branch — creating it (step 3) or checking it out (step 4) — first inspects `git status --porcelain` (which already excludes git-ignored files). If it reports **anything** — tracked modifications, staged changes, or untracked non-ignored files — the orchestrator **halts and asks the user** how to proceed with those pre-existing changes, offering exactly three choices, then acts and switches:
   - **proceed as-is** — attempt the create/checkout carrying the changes into the working tree. If git reports the switch would overwrite or conflict, the changes cannot be cleanly carried: re-present *stash* / *revert*.
...
- **Merge:** `gh pr merge <pr> --squash` — deliberately **without** `--delete-branch`. The local branch is checked out in your working directory, and git refuses to delete a branch that is currently checked out. Branch deletion belongs to Cleanup, below, which switches back to the default branch first so the ordering works.
- **Cleanup (idempotent — the resume table's Done row runs this same block; every step treats "already gone" as success):**
  1. Switch off the feature branch: `git switch <default>` (the resolved default branch). Already on it (e.g. a re-run) -> no-op success. Ignored scratch (e.g. SDD's ledger) does not block the switch; if *tracked* files have uncommitted modifications, halt and report instead — that is work the pipeline doesn't understand.
  2. Delete the local branch: `git branch -D <username>/<slug>`; treat "branch not found" as success. It must be `-D` — after a squash merge the branch is never "fully merged" in ancestry terms, so `-d` always refuses — and it must come after step 1, because git refuses to delete the currently checked-out branch.
  3. Delete the remote branch: `git push origin --delete <username>/<slug>`; treat "remote ref does not exist" as success (GitHub's auto-delete-head-branches may have won the race).
