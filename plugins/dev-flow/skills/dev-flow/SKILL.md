...
4. **Enter:** through the native worktree-entry tool's existing-path mode (e.g. `EnterWorktree` with `path`) when available, so the harness tracks the switch; else `cd <path>` and use absolute paths thereafter. All stage work happens inside the worktree. The orchestrator itself stays in the main checkout — its resume reads are `git show dev-flow/<slug>:<file>`, branch listings, and `gh`, none of which need entry.
