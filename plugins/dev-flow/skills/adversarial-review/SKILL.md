...
- When called by dev-flow, `working-dir` is the pipeline worktree's absolute path — the orchestrator passes it explicitly and invokes the review in-context (see dev-flow's worktree-entry rule).
